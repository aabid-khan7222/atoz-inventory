...
const SuperAdminDashboard = ({ activeMenu }) => {
  ...
  const loadDashboardData = async () => {
    ...
    setError(null);
    try {
      const [overview, salesAnalytics, inventoryInsights, services, transactions, financial] = await Promise.all([
  ...
  const renderSalesSectionHeader = (id, title) => (
    ...
    >
      ...
      <span style={{ color: 'var(--corp-text-secondary, #475569)', fontSize: '0.9rem' }}>
  ...
  const renderContent = () => {
    switch (activeMenu) {
      case 'dashboard':
        ...
        const { overview, salesAnalytics, inventoryInsights, services, recentTransactions, financial } = dashboardData;
        ...
        return (
          <div className="dashboard-content">
            ...
            <div className="stats-grid">
              ...
              <div className="stat-card metric-card-inventory">
                <div className="stat-info">
                  ...
                  <p className="stat-value">{formatCurrency(overview?.totalInventoryValue || 0)}</p>
                </div>
              </div>
              ...
              <div className="stat-card metric-card-customers">
                <div className="stat-info">
                  <h3>Total Customers</h3>
                  <p className="stat-value">{overview?.totalCustomers || 0}</p>
                </div>
              </div>
            ...
            {financial && (
              <div style={{ 
                ...
                <div className="financial-card" style={{ borderLeft: '4px solid #f59e0b' }}>
                  ...
                  <p className="subtext">
                    Margin: {financial.estimatedProfitMargin || 0}%
                  </p>
                </div>
              </div>
            )}
                ...
                <div className="chart-container">
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                    {salesAnalytics.topProducts.slice(0, 5).map((product, index) => {
                      ...
                      const cardBgHover = isDark ? '#334155' : '#f1f5f9';
                      ...
                      return (
                        ...
                        >
                          <div>
                            <div style={{ fontWeight: '600', color: textPrimary }}>{product.name}</div>
                            <div style={{ fontSize: '0.875rem', color: textMuted }}>{product.category}</div>
                          </div>
                          <div style={{ textAlign: 'right' }}>
                            <div style={{ fontWeight: '600', color: '#3b82f6' }}>{product.quantity} sold</div>
            ...
            {/* Active Services */}
            {services?.activeServices && services.activeServices.length > 0 && (
              <div className="content-section" style={{ marginTop: '2rem', borderLeft: '4px solid #3b82f6' }}>
                <h3 style={{ color: '#1e40af' }}>Active Services</h3>
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: '1rem' }}>
                  {services.activeServices.slice(0, 6).map((service) => (
                    ...
                    >
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'start', marginBottom: '0.75rem' }}>
                        <div style={{ fontWeight: '600', color: 'var(--corp-text-primary, #0f172a)' }}>{service.serviceNumber}</div>
                        <span className={`status-badge ${
                          service.status === 'completed' ? 'status-paid' : 
                      ...
                      <div style={{ fontSize: '0.875rem', color: 'var(--corp-text-secondary, #475569)' }}>
                        <div style={{ marginBottom: '0.25rem' }}>Customer: {service.customerName}</div>
                        <div style={{ marginBottom: '0.25rem' }}>Type: {service.serviceType}</div>
                        <div style={{ fontWeight: '600', color: 'var(--corp-text-primary, #0f172a)' }}>Charge: {formatCurrency(service.charge)}</div>
                      </div>
                    </div>
                  ))}
                </div>
        ...
        return (
          <div className="dashboard-content">
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem', gap: '1rem', flexWrap: 'nowrap' }}>
              <h2 style={{ margin: 0, whiteSpace: 'nowrap', flexShrink: 0 }}>Sales Management</h2>
              <select 
                value={period} 
                onChange={(e) => setPeriod(e.target.value)}
            ...
            {!salesLoading && !salesError && (
              <>
                <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap', marginBottom: '0.75rem' }}>
                  {(() => {
                    const isDark = theme === 'dark';
                    const inactiveBg = isDark ? '#1e293b' : '#fff';
                    const activeBg = isDark ? '#1e3a8a' : '#e0e7ff';
                      ...
                      <>
                        <button
                          ...
                          style={{ 
                            ...
                            background: openSalesSections.series ? activeBg : inactiveBg, 
                            color: textColor,
                            cursor: 'pointer' 
                          }}
                        >
                        ...
                        <button
                          ...
                          style={{ 
                            ...
                            borderRadius: '0.4rem', 
                            border: `1px solid ${borderColor}`, 
                            background: openSalesSections.product ? activeBg : inactiveBg, 
                            color: textColor,
                            cursor: 'pointer' 
                          }}
                        >
                        ...
                        <button
                          ...
                          style={{ 
                            ...
                            borderRadius: '0.4rem', 
                            border: `1px solid ${borderColor}`, 
                            background: openSalesSections.details ? activeBg : inactiveBg, 
                            color: textColor,
                            cursor: 'pointer' 
                          }}
                        >
                            ...
                            <tr key={`${item.invoiceNumber}-${item.sku}-${item.date}-${idx}`} style={{ borderBottom: '1px solid var(--corp-border-light, #eee)' }}>
                              ...
                              <td style={{ padding: '0.75rem', textAlign: 'right', fontWeight: '600', color: 'var(--corp-text-primary, #0f172a)' }}>
                                {item.isConfirmed ? (
                                  ...
                                  <td style={{ padding: '0.75rem', color: 'var(--corp-text-primary, #0f172a)' }}>
                                    {item.oldBatteryBrand || item.oldBatterySerialNumber || item.oldBatteryTradeInValue > 0 ? (
                                      <div style={{ fontSize: '0.875rem' }}>
                                        {item.oldBatteryBrand && (
                                          <div style={{ fontWeight: '600', color: '#3b82f6' }}>
                                            {item.oldBatteryBrand}
                                            {item.oldBatteryName && ` ${item.oldBatteryName}`}
                                          </div>
                                        )}
                                        {item.oldBatterySerialNumber && (
                                          <div style={{ color: '#64748b', fontSize: '0.75rem', marginTop: '0.25rem' }}>
                                            SN: {item.oldBatterySerialNumber}
                                          </div>
                                        )}
                                        {item.oldBatteryAhVa && (
                                        ...
                                        {item.oldBatteryTradeInValue > 0 && (
                                          <div style={{ color: '#059669', fontSize: '0.75rem', marginTop: '0.25rem', fontWeight: '600' }}>
                                            Value: {formatCurrency(item.oldBatteryTradeInValue)}
                                          </div>
                                        )}
                                      </div>
                                    ) : (
